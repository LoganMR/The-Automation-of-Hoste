...
def afterFees(source,amount):
    ...
    if source not in ['Airbnb','tripadvisor.com','flipkey.com']:
...
for k in range(len(RESERVATIONS)):
    FormRess[k]['CONFIRMATIONCODE'] = RESERVATIONS[k]['confirmationCode']

for k in range(len(RESERVATIONS)):
    CHI = datetime.datetime.strptime(RESERVATIONS[k]['checkIn'][0:10], '%Y-%m-%d').date().strftime('%m/%d/%Y')
    FormRess[k]['CHECKIN'] = CHI[0:6]+CHI[8:10]

for k in range(len(RESERVATIONS)):
    CHO = datetime.datetime.strptime(RESERVATIONS[k]['checkOut'][0:10], '%Y-%m-%d').date().strftime('%m/%d/%Y')
    FormRess[k]['CHECKOUT'] = CHO[0:6]+CHO[8:10]
...
for k in range(len(RESERVATIONS)):
    if (FormRess[k]['SOURCE'] == 'HomeAway') or (FormRess[k]['SOURCE'] == 'HomeAway Not Direct'):
        FormRess[k]['HOMEAWAY'] = round((FormRess[k]['CLEANINGFARE']+FormRess[k]['ACCOMMODATIONFARE'])*0.05,2)
    else:
        FormRess[k]['HOMEAWAY'] = 0

for k in range(len(RESERVATIONS)):
    if (FormRess[k]['SOURCE'] == 'Booking.com'):
        FormRess[k]['BOOKINGCOM'] = RESERVATIONS[k]['money']['hostServiceFee']
    else:
        FormRess[k]['BOOKINGCOM'] = 0

for k in range(len(RESERVATIONS)):
    FormRess[k]['NEWNETINCOME'] = round(afterFees(FormRess[k]['SOURCE'],FormRess[k]['TOTALPAYOUT'])-(FormRess[k]['CLEANINGFARE']+FormRess[k]['CITYTAX']+FormRess[k]['LOCALTAX']+FormRess[k]['HOMEAWAY']+FormRess[k]['BOOKINGCOM']+FormRess[k]['BOOKINGFEE']),2)
...
for k in range(len(RESERVATIONS)):
    FormRess[k]['BALANCEDUE'] = RESERVATIONS[k]['money']['balanceDue']
...
ws1['N1'] = 'SOURCE 6'
ws1['O1'] = 'SOURCE 7'
ws1['P1'] = 'DESCRIPTION 1'
ws1['Q1'] = 'DESCRIPTION 2'
ws1['R1'] = 'DESCRIPTION 3'
ws1['S1'] = 'DESCRIPTION 4'
ws1['T1'] = 'DESCRIPTION 5'
ws1['U1'] = 'DESCRIPTION 6'
ws1['V1'] = 'DESCRIPTION 7'
ws1['W1'] = 'HOMEAWAY COMMISSION'
ws1['X1'] = 'BOOKINGDOTCOM COMMISSION'
ws1['Y1'] = 'GUEST BOOKING FEE'

AR1 = ['CHECKIN','CHECKOUT','LISTINGSNICKNAME','GUESTSNAME','CLEANINGFARE','NEWNETINCOME','CITYTAX','LOCALTAX','SOURCE','SOURCE','SOURCE','SOURCE','SOURCE','SOURCE','SOURCE','DESCRIPTION','DESCRIPTION','DESCRIPTION','DESCRIPTION','DESCRIPTION','DESCRIPTION','DESCRIPTION','HOMEAWAY','BOOKINGCOM','BOOKINGFEE']
...
while finder>-1:
    giver=finder
    finder=FNameOut.find('/',finder+1)
bdsname2 = FNameOut[giver+1:len(FNameOut)-5]
bdsname1 = FNameOut[0:giver+1]

googledriveapi.uploadToFolder(FNameOut,'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet','1L9679QhwAhxZUHg7Krf1f-UkoaBgOYxx',bdsname2)

revbalduestr = "The following have balances due between " + dates[2] + " and " + dates[3] + ":\n"
for k in FormRess:
    if k['BALANCEDUE'] > 0:
        revbalduestr = revbalduestr + k['GUESTSNAME'] + ' ('+k["CONFIRMATIONCODE"]+')\n'

bdsname = "Balance Dues for " + dates[2] + ' through ' + dates[3] + ".txt"
bds=open('storedInfo/'+bdsname, 'w')
bds.write(revbalduestr)
bds.close()
print(revbalduestr)

googledriveapi.uploadToFolder(bdsname1+bdsname,'text/plain','1u64pawjqiWKDo_UaQ_QOzqOO--iSUWr8',bdsname2)

input("The excel sheet and the list of balance dues have been uploaded to the accounting drive\nPress Enter to close")
